...
results_movementBA <- list() # to save the model simulation results

extinction_probability <- vector() # to save the extinction probability

movementBA_vector <- seq(0.001,1,by = (1-0.001)/9) # 10 values of seasonality amplitude



for(movementBA_indx in 1:10){ # looping over seasonality amplitude
  ...
  while(num_stoch_real <= 200){ # looping over the number of stochastic realization
    ...
    params <- list(
      ...
      beta_B_base = 5, 
      phi_AB_base = 0.4, 
      phi_BA_base = movementBA_vector[movementBA_indx],
      seasonality = 0.5
    )
  ...
  results_movementBA[[movementBA_indx]] <- extinction_probability  # saving persistence probability as a function of seasonality amplitude
}
...
boxplot(results_seasonality,
        xlab = "Movement rate from patch B to A",
        ylab = "Disease persistence probability",
        names = c(0.001, 0.112, 0.223, 0.334, 0.445, 0.556, 0.667, 0.778, 0.889, 1.000),
